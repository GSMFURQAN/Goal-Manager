...
export const getTodos = async (req, res) => {
  ...
  if (params.dayView) {
    ...
    if (params.dayView == "all") {
      all = await Todo.find({ ...userIdCondition, done: false }).sort({
        dueDate: 1,
      });
...
export const deleteTodo = async (req, res) => {
  try {
    ...
    res.status(201).json("Goal deleted Successfully");
  } catch (error) {
    console.log("error saving the data in db", error);
    res.status(409).json({ message: error.message });
  }
...
export const deleteManyTodo = async (req, res) => {
  console.log("ddd", req.params.keyword);
  try {
    await Todo.deleteMany({
      "category.label": req.params.keyword,
